...
| Add TABL/DS create routing (Michael's report — #285 follow-up) | `src/handlers/intent.ts` (`normalizeWriteObjectType` + `TABL_WRITE_SUBTYPES` Set preserve TABL/DT and TABL/DS through SAPWrite normalization; `SAPWRITE_TABL_ALIAS` remaps the legacy `STRU/DS` alias to `TABL/DS` for SAPWrite only; `canonicalTablType()` collapses slash forms back to bare `TABL` for downstream Set checks (DDIC hints, RAP preflight, CDS dependency hints, cache invalidation). `objectBasePath` cases for both slash forms — `TABL/DT` → `/sap/bc/adt/ddic/tables/`, `TABL/DS` → `/sap/bc/adt/ddic/structures/`. `buildCreateXml` cases emit `adtcore:type="TABL/DT"` vs `"TABL/DS"` in the same `<blue:blueSource>` envelope. Update/delete/activate/edit_method/scaffold_rap_handlers for any TABL form route through `resolveTablObjectUrlForWrite()` so the PR #286 search-first safety contract applies uniformly. PR #286 discovery-gated create refusal scoped to bare `TABL` + `TABL/DT` — `TABL/DS` skips the gate because `/structures` exists on every release.) `src/handlers/schemas.ts` (`SAPWRITE_TYPES_ONPREM` + `SAPWRITE_TYPES_BTP` enums extended; read/write symmetry guard excludes the TABL slash forms). `src/handlers/tools.ts` (enums + SAPWrite description). Tests: `tests/unit/handlers/{schemas,intent}.test.ts` for normalization preservation + STRU/DS alias + routing + envelope + mixed-batch + read-path collapse + canonical-type Set checks; `tests/integration/adt.integration.test.ts` for end-to-end TABL/DS create with a long Z-name (~27 chars — exceeds the 16-char transparent-table limit so the routing decision is observable; same shape as Michael's `/LEOWM/SD_MON_S_WHO` report). Read paths unchanged (`SLASH_TYPE_MAP` still collapses both slash forms to bare `TABL` for SAPRead/SAPNavigate). |
