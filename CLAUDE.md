...
| Modify SAPTransport.create / transport target (TR_TARGET) / layer discovery | `src/adt/transport.ts`: **`createTransportWithTarget`** sets the explicit Transportziel/`TR_TARGET` (`C11`, `C11.021`, `/TRG/`) via the `tm:root`/`newrequest` endpoint (`POST /cts/transportrequests`, `tm:target=…`, `Content-Type: text/plain`) — the ONLY ADT path that sets the target; `CreateCorrectionRequest` (`/cts/transports`) silently ignores any `<TARGET>` (verified). SAP validates server-side: unknown target → HTTP 400 "Target 'X' does not exist". Group/`<sys>.<cli>` forms need extended transport control (CTC). `createTransport` (CreateCorrectionRequest, DEVCLASS `$TMP`) is the no-target path; optional `transportLayer` → `?transportLayer=` (indirect). `listTransportLayers` GETs `/packages/valuehelps/transportlayers` (parser arrays leaf `data`, leaves entities encoded — unwrap+decode). `src/handlers/intent.ts` (`handleSAPTransport` `create`: `target` → `createTransportWithTarget` with friendly "does not exist" hint, else package/layer path; reports resolved target via `getTransport`; `layers` discovery case), `src/handlers/{tools,schemas}.ts` (`target` + `transportLayer` params, `layers` action), `src/authz/policy.ts` (`SAPTransport.layers` read scope), `tests/unit/adt/transport.test.ts` + `tests/{unit/handlers,integration}/…`. **`tm:root`/`newrequest` works on 7.58 (the "fails on NW 7.50" code note is release-specific). a4h has no real targets (only `LOCAL`), so `/TRG/` is correctly rejected there — verify the happy path on a CTC system.** |
