...
| Add an `AdtClient` instance field / modify `withSafety()` clone | `src/adt/client.ts` (`withSafety()` builds the per-request safety clone via `Object.create(AdtClient.prototype)`, which **bypasses the constructor** — so EVERY instance field must be re-attached by hand via `Object.defineProperty`. A field added to the class but NOT to `withSafety()` is `undefined` on every clone and crashes the authenticated HTTP path the moment it's accessed). Caches (`tablUrlCache`, `tablWriteUrlCache`) and `packageHierarchyResolverHolder` are shared **by instance** (same reference, not a copy) so resolutions are visible across the original and all clones. Test in `tests/unit/adt/client.test.ts` (`describe('withSafety')` — assert the new field is the same instance on the clone). Regression that motivated this rule: issue #333 — missing `tablWriteUrlCache` → `TypeError: Cannot read properties of undefined (reading 'get')` on TABL write/activate under XSUAA/OIDC scopes or an API-key profile (stdio is exempt — it uses the constructor-built client). |
