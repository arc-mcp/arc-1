...
| `SAP_ALLOWED_PACKAGES` / `--allowed-packages` | Restrict writes to packages (default `$TMP`). Supports exact (`ZFOO`), prefix (`Z*`), DEVCLASS subtree (`ZFOO/**` via `TDEVC.PARENTCL`), and `*`. Subtree resolution is lazy + TTL-cached, fail-closed. Reads are never package-gated. |
...
| Modify `allowedPackages` pattern syntax (exact / `Z*` / `ZFOO/**` subtree) | `src/adt/safety.ts` (`decidePackageAllowed` + `checkPackage` + `intersectList.covers`), `src/adt/package-hierarchy.ts` (resolver + TTL cache, fail-closed), `src/adt/client.ts` (`getSubpackages` + `getPackageHierarchyResolver` holder shared across clones), `src/handlers/intent.ts` (resolver at every `checkPackage`; `invalidatePackageHierarchy()` on package mgmt), `src/adt/{gcts,abapgit}.ts`, tests. Subtree match needs runtime BFS via `POST /sap/bc/adt/repository/nodestructure` (`parseSubpackageNodestructure`); sync `isPackageAllowed` returns false for non-root subtree hits — only async `checkPackage` resolves them. |
