...
| Modify SAPTransport.create endpoint or DEVCLASS default | `src/adt/transport.ts` (`createTransport` POSTs `/sap/bc/adt/cts/transports`; defaults DEVCLASS to `$TMP` when `targetPackage` omitted), `src/handlers/intent.ts` (`handleSAPTransport` case `create`), `src/handlers/tools.ts`, tests. |
