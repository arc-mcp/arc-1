...
| `ARC1_RATE_LIMIT` / `--rate-limit` | **Layer 2 rate limit.** Per-user cap on MCP tool calls in req/min (default `60`). Key resolution walks the most-specific identity claim first via `resolveRateLimitUserKey()` in `src/server/mcp-rate-limit.ts`: `extra.userName` → `extra.email` → `extra.sub` → `extra.preferred_username` → `clientId` → `'__anon__'`. The `extra.sub` step is critical for OIDC deployments where every user shares the same `clientId` (= `azp`); without it, OIDC users would collapse into one bucket. Stdio mode (no `authInfo`) exempt. Returns MCP tool error with `retryAfter` (NOT HTTP 429, so the agent loop backs off cleanly). Set `0` to disable. Emits `mcp_rate_limited` audit event on denial. |
