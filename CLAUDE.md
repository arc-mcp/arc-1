...
| `SAP_ALLOWED_PACKAGES` / `--allowed-packages` | Restrict write operations to packages (default: `$TMP`). Supports exact (`ZFOO`), prefix wildcards (`Z*`), DEVCLASS subtree (`ZFOO/**` — `ZFOO` and every transitive sub-package via `TDEVC.PARENTCL`), and `*` (any). Subtree resolution is lazy + TTL-cached and fail-closed on any SAP-side failure. Reads are never package-gated. |
...
│   ├── class-structure.ts      # Class-section surgery splice + diff helpers (issue #303)
...
│   └── grep.ts                 # Regex match + ±context over source (SAPRead grep)
├── cache/
...
| Add SAPWrite TABL subtype (TABL/DT vs TABL/DS create routing — #285 follow-up) | `src/handlers/intent.ts` (`normalizeWriteObjectType`, `SAPWRITE_TABL_ALIAS`, `canonicalTablType`, `objectBasePath` + `buildCreateXml` cases per subtype; SAPWrite update/delete/activate for any TABL form goes through `resolveTablObjectUrlForWrite()`), `src/handlers/schemas.ts` (enums), `src/handlers/tools.ts`. Reads still collapse slash forms to bare `TABL` via SLASH_TYPE_MAP. See `docs/plans/completed/fix-tabl-ds-create-routing.md`. |
...
| Read FUGR full code tree (`SAPRead type=FUGR expand_includes=true`) | `src/adt/client.ts` (`getFunctionGroupExpanded` — BFS over the include graph; depth cap 5 / block cap 80; `seen` cycle+dedup guard; skips comment-only INCLUDE lines via `/^[^*\n]*\bINCLUDE\s+(\S+?)\s*\./gim`), `src/handlers/intent.ts` (`case 'FUGR'` expand branch calls it, emits `=== name ===` blocks + a `[truncated]` note). Recursion is required: `FUNCTION…ENDFUNCTION` bodies live in nested `LZ<grp>U01/U02` includes pulled from UXX — a one-level walk misses them. **Dynpros + GUI status are NOT reachable** — ADT has no REST for them (SE51/SE41 only; `/programs/programs/SAPL<grp>/dynpros/...` and `.../cuastatus/...` return 404, live-verified on a4h). Tests: `tests/unit/adt/client.test.ts` (recursion/dedup/placeholder), `tests/integration/adt.integration.test.ts` ("getFunctionGroupExpanded"). |
...
| Modify CLAS include writes | `src/handlers/intent.ts` (`SAPWrite update` branch with `include=definitions|implementations|macros|testclasses`, parent class lock + include URL PUT), `src/handlers/schemas.ts` (`SAPWRITE_CLAS_INCLUDES` + include guard), `src/handlers/tools.ts`, `tests/unit/handlers/intent.test.ts`, `tests/unit/handlers/schemas.test.ts`, `tests/unit/handlers/tools.test.ts`. **Auto-init (issue #303 follow-up):** the branch calls `safeUpdateClassInclude` (`src/adt/crud.ts`) which GET-probes the include and, when missing (notably `testclasses`/CCAU on a fresh class — `GET` 404, content PUT alone 500s with "…CCAU does not have any inactive version"), POST-creates it under the same class lock before the PUT (live-verified: empty `POST …/includes/{inc}?lockHandle=` → 201). `classifySapDomainError` maps the 500 marker to the `include-not-initialized` category (`src/adt/errors.ts`) as defense-in-depth for paths that bypass auto-init. Tests: `tests/unit/adt/crud.test.ts`, `tests/unit/adt/errors.test.ts`, `tests/integration/adt.integration.test.ts` ("testclasses include auto-init"). |
...
| Modify SAPTransport.create / transport target (TR_TARGET) / layer+target discovery | `src/adt/transport.ts`: **`createTransportWithTarget`** sets the explicit Transportziel/`TR_TARGET` (`C11`, `C11.021`, `/TRG/`) via `tm:root`/`newrequest` (`POST /cts/transportrequests`, `tm:target=…`, `text/plain`) — the ONLY ADT path that sets the target; `CreateCorrectionRequest` (`/cts/transports`) silently ignores `<TARGET>`. **`supportsExplicitTransportTarget(http)`** is the discovery-gate (the same capability SAP's Eclipse checks): true iff `/cts/transportrequests` advertises the `transportorganizer.v1+xml` Accept type — present on 7.58, absent on 7.50 (NW 7.50/7.51's TM stack rejects `newrequest` with "user action is not supported"). Uses `http.hasDiscoveryData()`/`discoveryAcceptFor()` over the already-cached discovery map. `listTransportTargets` GETs the official `/cts/transportrequests/valuehelp/target`; `listTransportLayers` GETs `/packages/valuehelps/transportlayers`; both via shared `parseNamedItems` (unwraps array leaf `data`, decodes entities incl. `&apos;`). `src/handlers/intent.ts` (`handleSAPTransport` `create`: empty-target reject → discovery-gate (fail-fast w/ SE09/SE10 guidance) → `createTransportWithTarget` (post-hoc "does not exist"→action="targets" hint + "user action not supported" fallback) else package/layer path; `layers`+`targets` discovery cases, both gated/graceful — NW 7.50 returns the target value-help as HTTP 200 empty, so gate on `supportsExplicitTransportTarget`, not status), `src/handlers/{tools,schemas}.ts` (`target`/`transportLayer` params, `layers`/`targets` actions), `src/authz/policy.ts` (`SAPTransport.{layers,targets}` read), `src/adt/http.ts` (discovery accessors). **Verified live: a4h 7.58 `target="DEV"` (from value-help) → 201 w/ target set; npl 7.50 → gate fail-fast, no POST. Research (Eclipse/adt-ls decompile + SAP Notes 2046730/2047506/2558101/2982187): no ADT way to set a target on 7.50; no modify-target-on-existing anywhere.** |
...
| Add/extend SAPRead `grep` regex search | `src/context/grep.ts` (pure `grepSource(source, pattern, {methods})` — `gim` regex with literal fallback, ±3 context, 100-match cap, `containingClass=>method` annotation), `src/handlers/intent.ts` (`grepText` helper + `if (args.grep)` branch in each source-bearing read case; CLAS branch fetches raw section via `client.getClassInclude` and annotates via `listMethods`, and rejects `grep`+`method` together), `src/adt/client.ts` (`getClassInclude` — raw section source, no `=== inc ===` wrapper), `src/handlers/{schemas,tools}.ts` (`grep` param), tests `tests/unit/context/grep.test.ts`, `tests/unit/handlers/{intent,schemas,tools}.test.ts`, `tests/unit/adt/client.test.ts`, `tests/e2e/sapread-grep.e2e.test.ts`. Issue #313; complements #307 surgery. |
...
| Add CLAS class-section surgery action (`edit_class_definition` / `add_method` / `edit_method_signature` / `delete_method` / `change_method_visibility`, issue #303) | `src/handlers/intent.ts` (five switch cases in `handleSAPWrite` after `edit_method`; `change_method_visibility` moves a METHODS clause between sections via `moveMethodDefinition`, IMPLEMENTATION untouched — the body-preserving alternative to `delete_method`+`add_method`), `src/adt/class-structure.ts` (pure splice + diff helpers: `spliceClassDefinition`, `spliceMethodSignature`, `insertMethodPair`, `removeMethodPair`, `moveMethodDefinition`, `findSectionAnchor`, `diffMethodSets`, `parseDefinitionBlockDeclarations`, `extractMethodNameFromClause`), `src/adt/client.ts` (`getClassStructure` GETs `/sap/bc/adt/oo/classes/{name}/objectstructure`), `src/adt/xml-parser.ts` (`parseClassStructure` — handles 7.50 split `CLAS/OO`+`CLAS/OM` and 7.58+ unified `CLAS/OM` shapes), `src/adt/types.ts` (`ClassStructure`, `MethodStructure`, `AttributeStructure`, `LineRange`), `src/handlers/schemas.ts` (new actions + `visibility`/`abstract` fields, `validateSapWriteInput` expands include-aware action set), `src/handlers/tools.ts` (action enum + properties + `Class-section surgery` description block), `src/authz/policy.ts` (four `SAPWrite.<action>` entries), tests under `tests/unit/adt/`, `tests/unit/handlers/`, `tests/integration/class-section-surgery.integration.test.ts`, `tests/e2e/class-section-surgery.e2e.test.ts`, fixtures `tests/fixtures/xml/objectstructure-clas-{a4h-758,npl-750}.xml`. Refuse-policy diff happens client-side BEFORE PUT: adds without IMPL stub + orphan IMPLs surface as structured errors with `add_method`/`delete_method` hints. ABSTRACT/EVENTS/INTERFACES/ALIASES exempt. `include=` writes skip the refuse-policy. Verified live on a4h S/4HANA 2023 (kernel 7.58) end-to-end; NPL 7.50 read parity verified via captured fixture. |
...
| Modify `allowedPackages` pattern syntax (exact / `Z*` / `ZFOO/**` subtree) | `src/adt/safety.ts` (`decidePackageAllowed` + `checkPackage` + `intersectList.covers`), `src/adt/package-hierarchy.ts` (resolver + TTL cache; fail-closed on resolution error), `src/adt/client.ts` (`getSubpackages` + `getPackageHierarchyResolver` lazy holder shared across `withSafety()` clones), `src/handlers/intent.ts` (pass resolver to every `checkPackage` call site; `invalidatePackageHierarchy()` on `create_package`/`delete_package`/`change_package` to drop stale subtrees), `src/adt/gcts.ts`/`abapgit.ts` (resolver passed through clone/createBranch/createRepo). Subtree match requires runtime BFS via the ADT `POST /sap/bc/adt/repository/nodestructure` endpoint (parsed by `parseSubpackageNodestructure` in `src/adt/xml-parser.ts`; verified live against `TDEVC.PARENTCL`). The previous `informationsystem/search?packageName=X&objectType=DEVC/K` shape silently ignores `packageName` and returns ~1000 unrelated DEVC rows — see `docs/research/package-subtree-endpoints.md`. `isPackageAllowed`'s sync boolean form returns false for non-root subtree hits — only `checkPackage` (async) resolves them. Tests: `tests/unit/adt/{safety,package-hierarchy,client,xml-parser}.test.ts`, `tests/unit/handlers/intent.test.ts` ("subtree rules" suite), `tests/integration/adt.integration.test.ts` ("getSubpackages (repository/nodestructure)" suite — live SABP_UNIT/AIF coverage). |
