...
│   ├── tool-registry.ts        # single source of per-tool type tables (ONPREM/BTP arrays derived from one {type,btp} table)
...
- **Error types**: `AdtApiError` (SAP HTTP), `AdtSafetyError` (blocked by config), `AdtNetworkError` (connectivity). `dispatch.ts` formats them with LLM-friendly hints.
...
- **Tool schema three-file sync** — every property must exist in `tools.ts` (JSON Schema for LLMs), `schemas.ts` (Zod), and the per-tool handler module (handler). A property missing from `tools.ts` is invisible to LLMs. Batch schemas (`batch_create` items) live separately from the top-level schema — update both.
