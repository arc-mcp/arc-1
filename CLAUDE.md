...
| Add TABL/DS create routing (Michael's report — #285 follow-up) | `src/handlers/intent.ts` (`normalizeWriteObjectType` + `TABL_WRITE_SUBTYPES` Set preserve TABL/DT and TABL/DS through SAPWrite normalization; both single-create and batch_create call sites use it. `objectBasePath` cases for both slash forms — `TABL/DT` → `/sap/bc/adt/ddic/tables/`, `TABL/DS` → `/sap/bc/adt/ddic/structures/`. `buildCreateXml` cases emit `adtcore:type="TABL/DT"` vs `"TABL/DS"` in the same `<blue:blueSource>` envelope. PR #286 discovery-gated refusal scoped to bare `TABL` + `TABL/DT` — `TABL/DS` skips the gate because `/structures` exists on every release.) `src/handlers/schemas.ts` (`SAPWRITE_TYPES_ONPREM` + `SAPWRITE_TYPES_BTP` enums extended; read/write symmetry guard excludes the TABL slash forms). `src/handlers/tools.ts` (enums + SAPWrite description). Tests: `tests/unit/handlers/{schemas,intent}.test.ts` for normalization preservation + routing + envelope assertions; `tests/integration/adt.integration.test.ts` for end-to-end TABL/DS create with `/LEOWM/`-style namespaced name (reproduces Michael's failure and proves the fix). Read paths unchanged (`SLASH_TYPE_MAP` still collapses both slash forms to bare `TABL` for SAPRead/SAPNavigate). |
