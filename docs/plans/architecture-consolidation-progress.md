...
- [x] **Docs** CLAUDE.md + AGENTS.md refreshed for the split layout → **DONE**. The 47 (CLAUDE) / 12 (AGENTS) `intent.ts` pointers in the Codebase Structure tree + Key Files table + prose now point at the owning module (read/write/write-helpers/dispatch/object-types/…); the type-list rows note the single-source `*_TYPE_TABLE`. AGENTS.md remains a partial stale fork of CLAUDE.md — its full consolidation (thin pointer vs. sync) is part of the deferred knowledge-architecture decision.
