...
## Paste-able #664 reply (review before posting; origin = `marianfoo`)

```markdown
Confirmed — this is a bug in ARC-1, not in your config or your prompt. Thanks for the unusually good
report; the exact error text made it reproducible in minutes.

**What's happening.** v1.0.0 added two FUNC-only fields to `SAPWrite` — `processingType` and
`updateTaskKind` (#634) — as plain enum strings with no `null` in their type, plus a validator that
hard-rejects them whenever `type != "FUNC"`. Clients that fill in **every** advertised schema property
(OpenAI/GPT strict mode, and the MCP clients that emulate it) have no way to say "absent" for those
two, so the model picks an enum member — `"normal"` / `"startImmediate"` — and ARC-1 then rejects the
whole call. ARC-1 never populates them itself; they arrive on the wire. That is also why
`ARC1_LOG_HTTP_DEBUG` produced nothing: the call dies at input validation, before any SAP request.

Reproduced live on SAP_BASIS 816 with your exact payload:

- your JSON **+** `processingType:"normal"`, `updateTaskKind:"startImmediate"` → your error, verbatim
- your JSON as written → `Created PROG ZPLU_HELLO_WORLD in package $TMP and wrote source code.`

It isn't PROG-specific — it breaks every `SAPWrite` type for an affected client, including real FUNC
creates.

**Workaround right now, no upgrade needed:** set `ARC1_SCHEMA_NULLABLE_OPTIONALS=on`. That restores the
`["string","null"]` form for optional fields, so your client can send `null` — which ARC-1 already
strips before validation.

**Fix in flight:** ARC-1 will drop inapplicable FUNC processing metadata before validation instead of
rejecting the call, the same way it already handles a stray `include` on a non-CLAS write (#360). Real
FUNC creates keep their values. PR linked below; it'll go out in the next release.
```
