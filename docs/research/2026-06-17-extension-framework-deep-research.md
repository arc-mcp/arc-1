...
- **Phase 2 (minimal)** — local trusted JS loader: `ARC1_PLUGINS=/abs/path.js[,…]` (absolute, readable, plus same-owner/not-world-writable checks on POSIX; Windows ACLs; dynamic import + shape validation; fail-fast). **This is what makes it dev-usable.**
