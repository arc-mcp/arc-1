...
> risk register (§5) current as findings are fixed or added.
...
Where: [`src/cache/`](../src/cache/) (source, dep-graph, inactive-list),
...
[`src/adt/package-hierarchy.ts`](../src/adt/package-hierarchy.ts), and the principal-propagation
failure path in [`src/server/server.ts`](../src/server/server.ts).
Fails when: an error path retries under a *different, higher* identity (the original R6 defect).
...
For production, the recommended baseline is one SAP identity model per ARC-1 instance. A PP instance
uses JWT auth with explicit `SAP_PP_STRICT=true`; API-key automation uses a separate non-PP instance
with a least-privileged technical SAP identity and its own safety ceiling. This separation is not
...
| R5 | **Audit sink redaction and private file sinks.** Original issue: redaction lived only in `StderrSink`, so file/BTP sinks could receive SAP source/error snippets (`resultPreview`, always-on `errorBody`, full bodies under `ARC1_LOG_HTTP_DEBUG`). | Med | all (file/btp sink) | [`logger.ts`](../src/server/logger.ts), [`audit.ts`](../src/server/audit.ts), [`sinks/file.ts`](../src/server/sinks/file.ts) | ✅ [#493](https://github.com/arc-mcp/arc-1/pull/493) central redaction + [#496](https://github.com/arc-mcp/arc-1/pull/496) private file mode merged |
| R6 | **Principal-propagation fallback identity.** Original issue: a failed JWT principal-propagation lookup could continue as the shared service account when `SAP_PP_STRICT=false`, changing both effective SAP privileges and audit identity. JWT PP failures now always return an error. API-key/non-JWT requests may still use the shared client because they never enter the JWT propagation path; explicit `SAP_PP_STRICT=true` rejects those mixed-auth calls too. Production guidance recommends strict PP-only and separate non-PP API-key instances, while supported mixed deployments are identified clearly in startup logs. | Med | PP only | [`config.ts`](../src/server/config.ts), [`server.ts`](../src/server/server.ts), [`server.test.ts`](../tests/unit/server/server.test.ts) | ✅ closed; default fail-closed shipped in [#488](https://github.com/arc-mcp/arc-1/pull/488), explicit JWT fallback removed |
| R7 | **TLS-off warning + diagnostics path encoding.** Original issue: `SAP_INSECURE` disabled TLS verification silently and diagnostics paths interpolated trace/dump IDs directly into ADT URLs. | Low/Med | all | [`config.ts`](../src/server/config.ts), [`diagnostics.ts`](../src/adt/diagnostics.ts), [`mta.yaml`](../mta.yaml) | ✅ [#491](https://github.com/arc-mcp/arc-1/pull/491) TLS-on descriptors + [#494](https://github.com/arc-mcp/arc-1/pull/494) warning/path encoding merged |
...
| R9 | **gCTS/abapGit `pull`/`push` package allowlist bypass.** Original issue: existing-repo pull/push/commit paths checked git write gates but did not revalidate the repository's real server-bound package. | Med (High shared-acct) | all (allowGitWrites) | [`abapgit.ts`](../src/adt/abapgit.ts), [`gcts.ts`](../src/adt/gcts.ts), [`git.ts`](../src/handlers/git.ts) | ✅ [#389](https://github.com/arc-mcp/arc-1/pull/389) abapGit + [#490](https://github.com/arc-mcp/arc-1/pull/490) gCTS merged |
| R10 | **`apply_quickfix` arbitrary authenticated POST under read scope.** Original issue: `proposalUri` was an unconstrained string POSTed verbatim with caller-controlled XML and gated as read. | Med | all | [`devtools.ts`](../src/adt/devtools.ts), [`policy.ts`](../src/authz/policy.ts), [`schemas.ts`](../src/handlers/schemas.ts) | ✅ [#489](https://github.com/arc-mcp/arc-1/pull/489) merged — quickfix URI allowlist + write scope/update op |
...
| R12 | **Historical: reverse-dependency (`usages`) index served cross-user with no per-user check.** The shared warmup index originally leaked where-used relationships across users. It was first gated in #393; the index and warmup path are now removed, and usages uses the caller's live SAP client. | Med | historical PP + warmup | [`where-used.ts`](../src/handlers/where-used.ts), [`context.ts`](../src/handlers/context.ts) | ✅ [#393](https://github.com/arc-mcp/arc-1/pull/393) merged; structural path removed 2026-07 |
...
| R14 | **Detailed SAP errors can be a recon oracle when explicitly enabled.** Minimal client-facing errors exist and are the HTTP default; stdio stays detailed for local diagnostics, and operators can still set `ARC1_MINIMAL_ERRORS=false` for trusted debugging. Detailed SAP error text can include another user's lock-owner **username** + transport id (T100 slots) and auth-object names, so do not disable minimal errors on shared or remotely reachable deployments. | Low | trusted debug / stdio / explicit opt-out | [`dispatch.ts`](../src/handlers/dispatch.ts), [`config.ts`](../src/server/config.ts) | ✅ [#495](https://github.com/arc-mcp/arc-1/pull/495) minimal mode + [#552](https://github.com/arc-mcp/arc-1/pull/552) HTTP default merged |
