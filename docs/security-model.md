...
For production, keep one SAP identity model per ARC-1 instance. A PP instance should use JWT auth
with explicit `SAP_PP_STRICT=true`; API-key automation should use a separate non-PP instance with a
least-privileged technical SAP identity and its own safety ceiling. Mixed mode remains a compatibility
feature, but it intentionally gives one endpoint both per-user and shared SAP audit identities.
