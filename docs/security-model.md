...
| B2 | ARC-1 request handling → ADT client | every handler dispatch | `handleToolCall` choke point: arg-normalize → scope (`ACTION_POLICY`) → Zod → package check — [`src/handlers/intent.ts`](../src/handlers/intent.ts), [`src/authz/policy.ts`](../src/authz/policy.ts) |
...
[`src/handlers/intent.ts`](../src/handlers/intent.ts).
...
[`src/handlers/intent.ts`](../src/handlers/intent.ts), SQL helpers in
...
| R6 | **Non-strict PP fallback runs as the shared service account.** `SAP_PP_STRICT=false` (default) falls through to `defaultClient` on PP failure — privilege escalation + wrong SAP-audit identity; inducible by forcing PP failure. | Med | PP only | [`server.ts:636`](../src/server/server.ts) | mitigable (`SAP_PP_STRICT=true`) |
...
- **OAuth DCR + state** — `client_id` is an HMAC-signed (HKDF, domain-separated) public identifier,
  ...
  rebuilt from parsed URL components (fixed in [#387](https://github.com/marianfoo/arc-1/pull/387);
...
- **2026-06-11** — Remediation landed on `main`: R8 ([#387](https://github.com/marianfoo/arc-1/pull/387)),
  R11 ([#388](https://github.com/marianfoo/arc-1/pull/388)), R9-abapGit ([#389](https://github.com/marianfoo/arc-1/pull/389)),
  R2 ([#392](https://github.com/marianfoo/arc-1/pull/392)), R1/R3/R12 ([#393](https://github.com/marianfoo/arc-1/pull/393)),
  R4 ([#394](https://github.com/marianfoo/arc-1/pull/394)), R16 ([#395](https://github.com/marianfoo/arc-1/pull/395)).
  **All High findings closed.** Still open (Med/Low): R5, R6, R7, R9-gCTS, R10, R13, R14, R15, R17.
  Updated the R8 "already-strong" bullet to reflect the fix. These fixes landed *after* the v0.9.13
  release, so they ship in the next release.
