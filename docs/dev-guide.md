...
| Headless CI ATC / AUnit (`SAPDiagnose action=atc_ci` / `unittest_ci`) | `src/adt/ci-quality.ts` (CSRF on dummy-run UUID, start, same-origin Location, bounded poll, result GET) + `src/adt/ci-quality-xml.ts` (OSL object set, Checkstyle/JUnit parsers), `src/handlers/diagnose.ts`, `{schemas,tools}.ts`, `src/authz/policy.ts` (`atc_ci` = read/Read, `unittest_ci` = read/Test). Not the IDE worklist/testrun APIs. Object set is packages / packageTrees / softwareComponents (AND if mixed). Default ATC variant `ABAP_CLOUD_DEVELOPMENT_DEFAULT`; `failOnSeverity` error\|warning\|info; AUnit `evaluateResults` default true (skipped does not fail). Poll timeout default 600s, clamp 30–1800. CLI `arc1-cli call SAPDiagnose --json … --output json` exits 1 when `fail===true`. Wire contract: `docs/research/2026-08-17-btp-ci-atc-aunit-communication-apis.md`. Product-neutral: no customer system/package names in ARC-1. |
