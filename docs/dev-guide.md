...
| Modify ATC check run (`SAPDiagnose action=atc`) | `src/adt/devtools.ts` (`runAtcCheck`) — **three-step flow**: `POST /atc/worklists?checkVariant=<v>` (an **empty** `checkVariant` runs the CI variant literally named `DEFAULT` — **not** `systemCheckVariant`; `resolveCheckVariant` therefore resolves `/atc/customizing` and always sends one, and rejects a name absent from `/atc/variants` because SAP would silently substitute `DEFAULT`; evidence: [dossier](research/2026-08-19-atc-default-check-variant.md)) returns the worklist id as text → `POST /atc/runs?worklistId=<id>` runs the checks and returns `FINDING_STATS` → poll `GET /atc/worklists/<id>` until the worklist XML, excluding SAP's per-GET root `timestamp`, has been unchanged for `ATC_SETTLE_QUIET_MS` (10 s). Any other body change resets the quiet period; counts alone are insufficient. Do NOT use `objectSetIsComplete=true` as a terminal marker: live 758 evidence changed from 23 findings/two objects to 73/ten after that flag was already true. Settling does not change the verdict (`complete` stays false, `evaluateAtc` still returns 3), only its timing; the reason is appended to `incompleteReasons`. Evidence: [dossier](research/2026-08-20-atc-completeness-polling.md). The check variant MUST be bound at worklist creation; the run request does not select checks. `maximumVerdicts` is ignored on 7.58 and is not used as a truncation signal. `parseAtcFindings` prefers `@_location` over position-less `@_uri`. Live-verified on A4H/758. |
