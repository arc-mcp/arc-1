...
| Modify ATC check run (`SAPDiagnose action=atc`) | `src/adt/devtools.ts` (`runAtcCheck`) — **three-step flow**: `POST /atc/worklists?checkVariant=<v>` (omit `checkVariant` → system default) returns the worklist id as text → `POST /atc/runs?worklistId=<id>` runs the checks and returns `FINDING_STATS` → poll `GET /atc/worklists/<id>` until its finding count matches that total. `objectSetIsComplete=true` is not fill-completeness evidence on 7.58. The check variant MUST be bound at worklist creation; the run request does not select checks. `maximumVerdicts` is ignored on 7.58 and is not used as a truncation signal. `parseAtcFindings` prefers `@_location` over position-less `@_uri`. Live-verified on A4H/758. |
