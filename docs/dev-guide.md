...
| Modify ATC check run (`SAPDiagnose action=atc`) | `src/adt/devtools.ts` (`runAtcCheck`) — **three-step flow**: `POST /atc/worklists?checkVariant=<v>` (an **empty** `checkVariant` runs the CI variant literally named `DEFAULT` — **not** `systemCheckVariant`; `resolveCheckVariant` therefore resolves `/atc/customizing` and always sends one, and rejects a name absent from `/atc/variants` because SAP would silently substitute `DEFAULT`; evidence: [dossier](research/2026-08-19-atc-default-check-variant.md)) returns the worklist id as text → `POST /atc/runs?worklistId=<id>` runs the checks and returns `FINDING_STATS` → poll `GET /atc/worklists/<id>` until the worklist **settles** — `ATC_SETTLED_POLLS` consecutive reads with an unchanged finding/processed-object count. Do NOT wait for `complete`: it ANDs six criteria a later poll can never change (root shape, worklist-id match, `objectSetIsComplete`, stats present, objects-container shape, priority validity), so waiting on it spends the caller's entire budget on identical re-reads — measured on NW 7.50 as wall-time == timeout at both 20 s and 45 s. Settling does not change the verdict (`complete` stays false, `evaluateAtc` still returns 3), only its timing; the reason is appended to `incompleteReasons`. Evidence: [dossier](research/2026-08-20-atc-completeness-polling.md). `objectSetIsComplete=true` is not fill-completeness evidence on 7.58. The check variant MUST be bound at worklist creation; the run request does not select checks. `maximumVerdicts` is ignored on 7.58 and is not used as a truncation signal. `parseAtcFindings` prefers `@_location` over position-less `@_uri`. Live-verified on A4H/758. |
