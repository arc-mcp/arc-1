...
| Add/modify npm audit / SAST / dep review CI gate | `.github/workflows/dependency-review.yml` (security and licenses separate), `.github/security-ruleset.json` (disabled template; only dependency security + CodeQL security), `.github/workflows/test.yml` (existing full-tree audit). Activation and native release-PR workflow approval: `docs/security-operations.md`. |
| Generate source/BTP dependency evidence | `scripts/security/{evidence,summary}.mjs`, `.github/workflows/dependency-evidence.yml`, `tests/unit/scripts/security-evidence.test.ts`. Lockfile-only root/AppRouter full/production inventories; no installation or credential collection; unavailable evidence is incomplete. CF staging stays customer-supplied. |
