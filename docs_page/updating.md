...
2. **Pin to a version** — in production, use exact version tags (for example `:0.9.13`), never `:latest`. Prevents surprise upgrades. <!-- x-release-please-version -->
...
<!-- x-release-please-start-version -->
```json
{ "command": "npx", "args": ["-y", "arc-1@0.9.13"] }
```
<!-- x-release-please-end -->
...
docker pull ghcr.io/marianfoo/arc-1:0.9.13
...
docker run -d --name arc1 -p 8080:8080 \
  --env-file .env \
  ghcr.io/marianfoo/arc-1:0.9.13
...
applications:
  - name: arc1-mcp-server
    docker:
      image: ghcr.io/marianfoo/arc-1:0.9.13   # ← update this
```
<!-- x-release-please-end -->
