...
2. **Pin to a version** — in production, use exact version tags (for example `:1.4.1`), never `:latest`. Prevents surprise upgrades. <!-- x-release-please-version -->
...
<!-- x-release-please-start-version -->
```json
{ "command": "npx", "args": ["-y", "arc-1@1.4.1"] }
```
<!-- x-release-please-end -->
...
docker pull ghcr.io/arc-mcp/arc-1:1.4.1
