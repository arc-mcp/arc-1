...
!!! warning "Principal propagation fails closed by default"
    With `SAP_PP_ENABLED=true`, JWT principal-propagation failures return an error instead of falling back to the shared service account. For production, set `SAP_PP_STRICT=true` explicitly and keep the instance JWT-only. Run API-key automation on a separate non-PP instance. See [Principal Propagation Setup](principal-propagation-setup.md).
...
- [ ] Per-user production instances set both `SAP_PP_ENABLED=true` and explicit `SAP_PP_STRICT=true`; API-key automation uses a separate non-PP instance
...
- [ ] Image pinned to an exact version (for example `:0.9.25`), not `:latest` <!-- x-release-please-version -->
