...
|---|---|---|
| Proxy response cleanup ([#805](https://github.com/arc-mcp/arc-1/issues/805)) | Fixes an ARC-1 1.2.0 regression where discarding a BTP Connectivity response body could terminate the process, including ordinary cached reads returning HTTP 304. Preserves response limits and caller-visible failures. | After upgrading to a release containing this fix, restore your previous cache setting if you used `ARC1_CACHE=none` only as a workaround. |
