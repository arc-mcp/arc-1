...
| `SKTD` / `KTD` | Knowledge Transfer Document attached to an ABAP object. Returns Markdown decoded from the ADT XML envelope, one `## <node id>` section per documented node when the object has several (BDEF entities, savers, actions, functions, …). Exact node-ID headings are reserved routing syntax. When SAP has pre-created nodes that nobody documented yet, the response ends behind a reserved HTML-comment marker with a compact, reconstructible index grouped by type. Add a `## <full id>` section above the marker to document one. `SAPWrite` ignores the marker and context below it; the writable Markdown still follows the requested active/inactive version semantics. `KTD` is a friendly alias; `SKTD` remains the canonical SAP object type. |
