...
- **`atc`** — Run ATC (ABAP Test Cockpit) checks. Returns findings with priority, check title, message, URI, line number, plus quickfix metadata (`quickfixInfo`, `hasQuickfix`). Optional `variant` parameter for custom check variants. Use `resultFormat="structured"` when automation must distinguish a complete result from a missing/false object-set completeness marker, a zero-object result, malformed priorities, or an asynchronously populated worklist that has not reached the finding total reported by the completed run.
