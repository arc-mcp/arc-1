...
Multi-target v1 includes `atc`, `atc_ci`, `unittest`, and `unittest_ci` under the existing `read` scope. They do not mutate
...
| `timeoutSeconds` | number | No | For `unittest` or `atc`: execution and verification budget; default `300`, range `1..3600`. Timeout is incomplete evidence, never a pass. For `atc_ci` / `unittest_ci`: poll timeout, default `600`, clamped `30..1800`. |
...
| `packages` | array | No | Object set for `atc_ci` / `unittest_ci`: package names. Mixing packages and software components is a logical AND. |
| `packageTrees` | array | No | Object set for `atc_ci` / `unittest_ci`: packages plus their subpackages. |
| `softwareComponents` | array | No | Object set for `atc_ci` / `unittest_ci`: software component names. |
...
| `title` | string | No | `unittest_ci`: optional run title. |
| `context` | string | No | `unittest_ci`: optional run context. |
| `ownTests` | boolean | No | `unittest_ci`: include own tests (default true). |
| `foreignTests` | boolean | No | `unittest_ci`: include foreign tests (default true). |
| `harmless` | boolean | No | `unittest_ci`: include harmless-risk tests (default true). |
| `dangerous` | boolean | No | `unittest_ci`: include dangerous-risk tests (default true). |
| `critical` | boolean | No | `unittest_ci`: include critical-risk tests (default true). |
| `short` | boolean | No | `unittest_ci`: include short-duration tests (default true). |
| `medium` | boolean | No | `unittest_ci`: include medium-duration tests (default true). |
| `long` | boolean | No | `unittest_ci`: include long-duration tests (default true). |
| `measurements` | string | No | `unittest_ci`: measurement type (default none). |
| `evaluateResults` | boolean | No | `unittest_ci`: fail on errors or failures (default true; skipped tests do not fail) |
...
- **`unittest_ci`** — Headless ABAP Unit via Communication Arrangement `SAP_COM_0735` (`/sap/bc/adt/api/abapunit/runs`). Scope is packages, package trees, and/or software components — not a single object URI. Returns structured tests/summary plus verbatim JUnit XML in `reportXml`. `evaluateResults` (default true) sets `fail` when `errors>0` or `failures>0`; skipped tests do not fail. Uses Basic `SAP_URL`/`SAP_USER`/`SAP_PASSWORD` (separate Communication User from ATC). Poll timeout `timeoutSeconds` (default 600).
...
- **`atc_ci`** — Headless ATC via Communication Arrangement `SAP_COM_0901` (`/sap/bc/adt/api/atc/runs?clientWait=false`). Same object-set inputs as `unittest_ci`. Default variant `ABAP_CLOUD_DEVELOPMENT_DEFAULT`. Returns structured findings plus verbatim Checkstyle XML in `reportXml`. `failOnSeverity` (`error` default) sets `fail` when any finding is at least that level; empty Checkstyle succeeds. Distinct from `action=atc` (IDE worklist).
...
SAPDiagnose(action="unittest_ci", softwareComponents=["/DMO/SWC"], evaluateResults=true)
