...
| `oldContent` | string | No | `edit_content`: exact non-empty source text to replace, without line-number prefixes. |
| `newContent` | string | No | `edit_content`: replacement text; empty string deletes the matched text. |
| `lineStart` | integer | No | `edit_content`: optional first line of the anchor search window (1-based, inclusive); requires `lineEnd`. |
| `lineEnd` | integer | No | `edit_content`: last line of the search window; requires `lineStart`. |
...
`SAPWrite(action="edit_content", type="PROG"|"INCL", name=..., oldContent=..., newContent=...)` replaces one exact occurrence. Empty `newContent` deletes it; zero or multiple matches refuse the edit unless the replacement is already uniquely present. Optional `lineStart`/`lineEnd` scope the anchor search. Read raw source first. FUGR include parents can be supplied with `group` or resolved through search.
