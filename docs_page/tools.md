...
| `where` | array | No | For TABLE_QUERY: ANDed `{field,op,value?}` conditions. Operators: `=`, `!=`, `<>`, `<`, `<=`, `>`, `>=`, `LIKE`, `NOT LIKE`, `IN`, `NOT IN`, `IS NULL`, `IS NOT NULL`. IN values are bare comma-separated values; ARC-1 quotes/escapes them. On 758 use `<>`, because accepted `!=` is sent unchanged and SAP rejects it. Also used by CLUSTER_READ to filter cluster keys (e.g. `RELID`/`LOG_HANDLE`). |
| `layout` | string | No | For CLUSTER_READ: `"applog"` (decodes BALDAT buckets as `BAL_S_MSG` messages), `"stxl"` (renders STXL as SAPscript text — the default when `name="STXL"`), a DDIC structure name applied to every object, or `"OBJ=STRUCT,OBJ2=STRUCT2"` to name a structure per object. Omit to get numbered fields with a note that names aren't in the cluster. |
| `schemaOnly` | boolean | No | For CLUSTER_READ: return each object's field types and row counts without the decoded row values. |
| `lang` | string | No | For CLUSTER_READ with `layout="applog"`: language for T100 message texts (ISO code or SAP key). Defaults to English. |
...
| `CLUSTER_READ` | Decodes ABAP data-cluster tables — `BALDAT`, `INDX`, `STXL`, or any Z table built the same way with `EXPORT ... TO DATABASE`. ADT's data preview returns the raw `CLUSTD` fragments as hex and never decodes them; ARC-1 joins the fragments per key over the same `runTableQuery` path `TABLE_QUERY` uses (server-built SELECT, `where` filters the cluster key, no free SQL) and decodes the SAP-internal binary format client-side. Field names are not stored in a cluster — the kernel writes types, not DDIC — so pass `layout` to name them: `"applog"` for BALDAT application-log messages, `"stxl"` for STXL SAPscript text (the default when `name="STXL"`), a DDIC structure name, or `"OBJ=STRUCT,OBJ2=STRUCT2"` per object. `schemaOnly` returns types/counts without row values. On-prem only — BTP ABAP Environment does not expose these classic kernel tables. Caveat: `runTableQuery` never sorts (`ORDER BY` is rejected on NW 7.50/7.51), so on a HANA-backed system an unfiltered scan of a busy multi-fragment table (e.g. `BALDAT`) can split a cluster's fragments across the `maxRows` boundary; only the last group is dropped as possibly-truncated, so an earlier split surfaces as a per-record `error` instead of wrong data. Filter `where` to one key (e.g. one `LOG_HANDLE`), or raise `maxRows` past the table's fragment count, for a reliable join. |
