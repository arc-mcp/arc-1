...
accepted. Transport mutations and topology actions are unavailable; `diff` is also
...
- **`release`** — Release a single request or task, then confirm its final CTS status. Requests are read back in `R` or `N`; released tasks may disappear from SAP's organizer tree and are then confirmed only after an accepted release report. Timeout, unknown state, or unexplained disappearance is an error. Use `resultFormat="structured"` for per-ID status checks.
...
Read each part's `baselineStatus` before summarising the change:
