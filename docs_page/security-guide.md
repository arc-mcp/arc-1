...
| `host_rejected` | An HTTP-transport request was blocked because its `Host` header is not in the allowlist (`ARC1_ALLOWED_HOSTS`, or the auto-derived loopback + `ARC1_PUBLIC_URL`/VCAP host list) — DNS-rebinding defense. Includes host, method, path. |
...
- **Auto-protect default.** With `ARC1_ALLOWED_HOSTS` unset, the check is active on **any** bind and accepts two host classes: the loopback values (`localhost`/`127.0.0.1`/`::1` — a rebind attacker cannot forge them, so local dev keeps working even on a `0.0.0.0` bind) and the **advertised public host** derived from `ARC1_PUBLIC_URL` / `VCAP_APPLICATION` (so a reverse-proxy or BTP deploy, which is reached via that exact host, works with no extra config). Everything else — the rebind payload — is rejected. A non-loopback bind with neither `ARC1_PUBLIC_URL` nor `ARC1_ALLOWED_HOSTS` set logs a startup warning and accepts loopback Hosts only.
- **Explicit allowlist.** `ARC1_ALLOWED_HOSTS=mcp.example.com,localhost,…` enforces a fixed set regardless of bind. Ports, case, IPv6 brackets, and trailing dots are normalized before comparison, so `mcp.example.com:443` and `mcp.example.com.` both match `mcp.example.com`. `ARC1_ALLOWED_HOSTS=*` disables the check entirely.
- **`/health` is exempt.** The health endpoint is hit directly on the bind address by infrastructure probes (CF Diego, Kubernetes `httpGet` liveness — which defaults the probe `Host` to the pod IP, load balancers), which send a non-loopback `Host` that no allowlist can anticipate. It returns only a static `{status:"ok"}` with no SAP state or secrets, so it is intentionally not Host-gated — every state- or auth-bearing route (`/mcp`, OAuth, UI) still is.
