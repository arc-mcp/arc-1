...
| `--blocked-data-sources` | `SAP_BLOCKED_DATA_SOURCES` | empty (off) | **Experimental.** Exact comma-separated table/CDS identities to deny on `SAPQuery`, `TABLE_QUERY`, and `TABLE_CONTENTS`. Values are trimmed, uppercased, deduplicated, and validated at startup. A non-empty list activates conservative SQL parsing plus live CDS/replacement-object lineage checks before every data request; direct or transitive matches are denied, and unresolved/unsupported lineage fails closed. |
...
!!! warning "Experimental blocklist boundary"
    `SAP_BLOCKED_DATA_SOURCES` is a deny emergency brake, not an isolation allowlist: every source not listed remains eligible. When active, ARC-1 accepts only SQL that its pinned ABAP parser can prove is one static `SELECT`/`WITH`; it refuses host/dynamic sources, privileged/client/connection clauses, association paths, malformed or unsupported syntax, classic DDIC views, CDS table functions, and filtered `TABLE_CONTENTS`. Use structured `TABLE_QUERY` for filtered reads. Each accepted request performs fresh ADT search, canonical table-source, and/or active CDS dependency-graph reads, so it is slower and the metadata check is not transactionally atomic with the later data POST. Replacement annotations are extracted with DDL comment/string awareness; malformed or duplicate active annotations fail closed. Dependency XML is rejected above 5,000,000 characters before parsing, then bounded to 64 levels and 1,000 nodes. On the verified SAP_BASIS 750 system, the generic TABL fallback omits replacement-object metadata; a known blocked CDS dependency is still denied, but otherwise-unblocked requests conservatively fail unresolved rather than trust that lossy source. The setting is global in multi-target mode. Empty/unset preserves existing behavior with zero new metadata calls. Keep SAP roles and CDS DCL as the primary controls.
