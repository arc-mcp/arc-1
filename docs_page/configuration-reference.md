...
| `--system-label` | `ARC1_SYSTEM_LABEL` | empty | Free-text label of the connected SAP system (e.g. `ERP production (read-only)`), prepended as the first line of the MCP server instructions. Complements `--server-name`: some clients replace the advertised name with an opaque connector id, and the label then remains the only way a model can tell several ARC-1 instances apart without a probing tool call. Not used in multi-target mode, which builds its own instructions from the discovered targets. |
