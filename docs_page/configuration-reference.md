...
| `--allowed-hosts` | `ARC1_ALLOWED_HOSTS` | (empty) | Comma-separated `Host`-header allowlist (include the port) for the HTTP transport — DNS-rebinding defense (SEC-14). Empty (default) auto-protects **loopback binds only** (a derived `localhost`/`127.0.0.1` allowlist); non-loopback binds (`0.0.0.0`, a reverse proxy, or the BTP gorouter — which already control `Host`) stay off unless set. `*` disables. See [security-guide.md §11](security-guide.md#11-network-security). |
