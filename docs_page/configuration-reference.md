...
| `--server-name` | `MCP_SERVER_NAME` | `arc-1` | Server name advertised in the MCP `initialize` handshake. Give each instance a unique name when running several ARC-1 instances against different SAP systems so MCP clients keep them distinct instead of collapsing them and assigning dynamic tool suffixes. |
