...
| `--ui[=MODE]` | `ARC1_UI` | `off` | Read-only browser console. `off` disables it. `local` starts a loopback sidecar UI at `ARC1_UI_ADDR` for stdio/Claude-style local use. `web` mounts `/ui` and `/ui/api/*` on the HTTP server. `true` maps to `local` for stdio and `web` for HTTP. |
...
The UI is inspection-only in v1: sanitized config, safety/auth state, feature-probe status, cache counts/source metadata, and recent sanitized audit events. It does not expose writes, config mutation, cache mutation, feature probes, or cached ABAP source bodies. In HTTP mode `/ui/api/*` requires an `admin`-scoped bearer token when API keys, OIDC, or XSUAA are configured; without HTTP auth it is open like `/mcp`.
