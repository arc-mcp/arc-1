...
| `--pp-strict` | `SAP_PP_STRICT` | `true` when PP is enabled | JWT PP failures always return an error and never change to the shared identity. Set explicitly to `true` for the recommended PP-only production topology; API-key / non-JWT tool calls are then rejected. Explicit `false` retains compatibility mixed mode, in which API keys use the shared client, but never enables JWT fallback. Prefer a separate non-PP instance for API-key automation. |
