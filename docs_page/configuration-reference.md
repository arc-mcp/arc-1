...
| `--pp-strict` | `SAP_PP_STRICT` | `true` when PP is enabled | JWT PP failures always return an error and never change to the shared identity. API-key / non-JWT requests still use the shared client by default because they do not enter the JWT PP path. Set explicitly to `true` when those non-JWT requests should also be rejected; explicit `false` retains mixed-auth behavior but does not enable JWT fallback. |
