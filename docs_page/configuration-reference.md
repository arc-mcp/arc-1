...
| `--adtls-destination` | `SAP_ADTLS_DESTINATION` | — | Reuse a connection already configured in SAP ADT for VS Code. Reads the named destination (`id`) from `~/.adtls/destinations.json` and seeds `SAP_URL` / `SAP_USER` / `SAP_CLIENT` / `SAP_LANGUAGE` **below** any explicit flag/env value. The password is never stored in that file (ADT keeps it in the OS secret store), so `SAP_PASSWORD` is still required. `reentranceTicket`/`oauth`/`sso` destinations only supply the URL — ARC-1's MCP server authenticates with Basic/cookie/BTP-OAuth, not a browser reentrance ticket. Fail-soft: an unknown id or missing file logs a warning and falls through to env/defaults. The file is read-only — ARC-1 never writes it. (#442) |
| `--adtls-destinations-file` | `SAP_ADTLS_DESTINATIONS_FILE` | `~/.adtls/destinations.json` | Override the path to the ADT-for-VSC destinations store. |
