...
| `--cimd-enabled` | `ARC1_CIMD_ENABLED` | `false` | Accept **Client ID Metadata Document** identities (CIMD, SEP-991) alongside DCR, and advertise `client_id_metadata_document_supported` in the OAuth metadata. The two move together on purpose: an MCP client that sees the flag stops using DCR, so advertising without resolving would break every capable client. Enabling this makes ARC-1 perform an outbound HTTPS fetch to a URL supplied by an unauthenticated caller on `/authorize` — the fetch is SSRF-hardened (address validation + connection pinning, redirects refused, 5 KiB streaming cap, no credentials), but on BTP this process sits beside a Cloud Connector, so review `docs/research/2026-08-18-cimd-client-identity.md` before turning it on. Only consulted when XSUAA auth is on. |
| `--cimd-allowed-hosts` | `ARC1_CIMD_ALLOWED_HOSTS` | unset (any HTTPS host) | Comma-separated allowlist for CIMD `client_id` hosts: exact (`claude.ai`) or single-label wildcard (`*.vscode.dev`). Empty means open, which is CIMD's point — admitting a client identity grants nothing by itself, since XSUAA still authenticates the user and MCP scopes, the safety ceiling, and the user's own SAP authorizations all still apply. Set it when egress policy requires a closed destination set, or to limit phishing surface: CIMD proves control of a domain and nothing more, so a document's `client_name` is attacker-chosen and must never be shown as a trusted identity. Enforced **before** any DNS or socket, so a rejected host produces no outbound traffic. |
| `--cimd-cache-ttl-seconds` | `ARC1_CIMD_CACHE_TTL_SECONDS` | `900` | Fallback lifetime for a cached CIMD document whose response carries no usable cache headers. Responses that do carry `Cache-Control`/`Expires` are honoured but clamped to `[300 s, 3600 s]`; this value is clamped to the same window. The floor stops a hostile `max-age=0` turning every `/authorize` into an outbound fetch; the ceiling bounds staleness after a client rotates its redirect URIs. The cache is per-instance and in memory, consistent with ARC-1's stateless design. |
| `--cimd-proxy-url` | `ARC1_CIMD_PROXY_URL` | unset (direct) | Forward proxy for CIMD document fetches, e.g. `http://proxy.corp:3128` (credentials supported and never logged). ARC-1 tunnels with `CONNECT <validated-ip>:<port>` rather than handing the proxy a hostname, so the proxy resolves nothing and the SSRF address validation survives the hop. Two consequences: a proxy that refuses `CONNECT` to a bare IP cannot be used (surfaces as a `proxy_refused` audit reason, never a silent downgrade), and a TLS-intercepting proxy fails certificate validation unless its CA is trusted via `NODE_EXTRA_CA_CERTS`. |
