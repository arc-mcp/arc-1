...
| `--allowed-hosts` | `ARC1_ALLOWED_HOSTS` | (empty) | Comma-separated `Host`-header allowlist for the HTTP transport — DNS-rebinding defense (SEC-14). Ports, case, IPv6 brackets, and trailing dots are normalized before compare. Empty (default) auto-protects **any bind**: the loopback Host values (`localhost`/`127.0.0.1`/`::1` — a rebind attacker cannot forge them) plus the advertised public host derived from `ARC1_PUBLIC_URL` / `VCAP_APPLICATION` are accepted, and arbitrary Hosts (the rebind payload) are rejected — including on a `0.0.0.0` bind. So local dev (via `localhost`), reverse-proxy (`ARC1_PUBLIC_URL`), and BTP (VCAP route) all work with no extra config. Set explicit public/proxy hosts here to add more; `*` disables. See [security-guide.md §11](security-guide.md#11-network-security). |
