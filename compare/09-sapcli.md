...
| 2026-04-27 | `d7a6f2d`/`df954a3` **Transaction (TRAN/T)** ADT mapper + CLI create (type-specific params) | **Med** | **GAP on write**: ARC-1 reads TRAN (VIT metadata) but has no write. Maps to roadmap **FEAT-62** (already tracking, triggered by sapcli PR #156); now a merged reference impl for the create envelope. | TODO (FEAT-62) |
...
| 2026-04-24 | `942d70b` AUnit **Branch + Procedure coverage** printing + `238be0b` `--skip-covered` | **High** | **GAP (tracked)**: ARC-1 runs unit tests with `coverage active="false"` hardcoded (`devtools.ts:554`) → pass/fail only. sapcli is now a reference impl. → roadmap **FEAT-31/FEAT-41**. | TODO |
| 2026-04-21 | `8a3f309`/`f072f38` DDL **API Release** ops + `874c3b3` `ddl apistate set --auth-values` | **Med** | **GAP on write**: ARC-1 *reads* API state (`getApiReleaseState`, FEAT-02) but cannot *set/release* the C1 contract. Now in BOTH sapcli AND vibing-steampunk → strong clean-core dual-signal. → roadmap (extend FEAT-02). | TODO |
