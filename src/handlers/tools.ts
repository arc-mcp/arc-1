...
const SAPREAD_DESC_ONPREM =
  ...
  'Version: source types default active; inactive requests a draft (SAP may return active if none); auto selects the developer view. DTEL/server-driven types default to developer view. Explicit SDO versions must match metadata. Active source reads note when a draft exists.';
...
const SAPREAD_DESC_BTP =
  ...
  'CLAS: method="*" for signatures, method="NAME" for one body, or grep. Global class declaration/implementation: MAIN (omit include). definitions/implementations contain local helpers. Details: docs_page SAPRead. ' +
  'grep: case-insensitive regex; returns matching lines, context and line numbers, with owning class/method for CLAS. ' +
  'Version: source types default active; inactive requests a draft (SAP may return active if none); auto selects the developer view. DTEL/server-driven types default to developer view. Explicit SDO versions must match metadata.';
      ...
      inputSchema: {
        ...
        properties: {
          ...
          version: {
            ...
            description:
              'Source defaults active; inactive requests a draft (SAP may return active if none); auto selects developer view. DTEL/server-driven types default to developer view. Explicit SDO versions must match metadata.',
          },
          includeSignature: {
            type: 'boolean',
            description:
