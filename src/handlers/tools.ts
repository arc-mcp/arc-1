        ...
        properties: {
          action: {
            ...
            description:
              ...
              'diff: what a transport CHANGED — per object, the revision written under it vs the one before, as diff hunks (get only lists names). LIMU entries roll up to their class; an open transport compares its work state to the last released revision. Read baselineStatus before summarising: prior-revision = solid, prior-revision-unverified = guessed pair, no-prior-snapshot = created here, baseline-unavailable = the read FAILED, never call that unchanged. ' +
