...
const SAPTRANSPORT_DESC_ONPREM =
  ...
  'get (details with tasks and objects), create (Workbench/Customizing request — SAP infers the type from the target package transport route, not a parameter; optional explicit target. Transport-of-Copies/T is not created here), ' +
      ...
      inputSchema: makeOptionalPropertiesNullable({
        ...
        properties: {
          ...
          refObjectType: {
            type: 'string',
            description:
              'SKTD/KTD create: ADT type+subtype of the parent object the KTD documents (e.g., "DDLS/DF", "BDEF/BDO", "SRVD/SRV", "SRVB/SVB", "DEVC/K"). Do not use CLAS/OC, INTF/OI, or PROG/P; use ABAP Doc for code objects. Required for SKTD/KTD create.',
          },
          refObjectName: {
            type: 'string',
