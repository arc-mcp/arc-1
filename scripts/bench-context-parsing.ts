...
function fixture(name: string, lines: number): string {
  return [
    `CLASS ${name} DEFINITION PUBLIC.`, 'PUBLIC SECTION.', 'CLASS-METHODS run.', 'ENDCLASS.',
    `CLASS ${name} IMPLEMENTATION.`, 'METHOD run.', 'DATA value TYPE i.',
    ...Array.from({ length: lines - 9 }, () => 'value = value + 1.'), 'ENDMETHOD.', 'ENDCLASS.',
  ].join('\n');
}
function elapsed(run: () => void): number { const started = performance.now(); run(); return performance.now() - started; }
function median(values: number[]): number { return [...values].sort((a, b) => a - b)[Math.floor(values.length / 2)]!; }
const reports = [];
for (const lines of [300, 1000]) {
  const sources = Array.from({ length: 20 }, (_, i) => ({ name: `ZCL_BENCH_${i}`, source: fixture(`ZCL_BENCH_${i}`, lines) }));
  for (const depth of [1, 2]) {
    const cache = new ContextParseCache();
    const baseline = () => sources.map(({ source, name }) => ({
      contract: extractContract(source, name, 'CLAS', Version.v758),
      deps: depth === 2 ? extractDependencies(source, name, true, Version.v758) : [],
    }));
    const cached = () => sources.map(({ source, name }) => ({
    ...
    reports.push({ dependencies: sources.length, sourceLines: lines, depth,
      validationAndWarmupMs: Math.round(coldMs), uncachedMedianMs: +median(uncached).toFixed(2),
      warmMedianMs: +median(warm).toFixed(2), retained: cache.stats() });
  }
}
