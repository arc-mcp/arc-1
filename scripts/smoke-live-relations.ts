...
const cases = z
  .array(LiveRelationsInput)
  .min(1)
  .max(8)
  .parse(JSON.parse(process.env.TEST_RELATION_CASES ?? '[]'));
const env: Record<string, string> = {
  PATH: process.env.PATH ?? '',
  HOME: process.env.HOME ?? '',
  SAP_URL: process.env.TEST_SAP_URL ?? '',
  SAP_USER: process.env.TEST_SAP_USER ?? '',
  SAP_PASSWORD: process.env.TEST_SAP_PASSWORD ?? '',
  SAP_CLIENT: process.env.TEST_SAP_CLIENT ?? '001',
  SAP_TRANSPORT: 'stdio',
  ...
  ARC1_PLUGINS: '',
  ARC1_CACHE: 'memory',
  ARC1_LOG_LEVEL: 'error',
  ARC1_TOOL_MODE: 'standard',
  ARC1_MULTI_TARGET_ENDPOINTS: 'false',
};
assert.ok(
  env.SAP_URL && env.SAP_USER && env.SAP_PASSWORD,
  'TEST_SAP_URL/USER/PASSWORD and TEST_RELATION_CASES are required.',
);
...
for (const enabled of [false, true]) {
  ...
  transport.stderr?.on('data', () => {
    /* Drain diagnostics without retaining SAP payloads. */
  });
  ...
  try {
    ...
    for (const input of cases) {
      ...
      assert.ok(
        value.nodes.length <= input.maxResults && value.edges.length <= 100 && value.metrics.httpAttempts <= 12,
      );
      report.push({
        input,
        nodes: value.nodes.length,
        edges: value.edges.length,
        expanded: value.expanded.length,
        truncated: value.truncated,
        reasons: value.truncationReasons,
        metrics: value.metrics,
      });
    }
    const absent = await client.callTool({
      name: 'SAPNavigate',
      arguments: { action: 'relations', type: 'CLAS', name: 'ZCL_ARC1_ABSENT_9F82D' },
    });
