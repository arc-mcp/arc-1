...
/**
 * The gate used to be consulted synchronously, so a cold discovery map (always, in the CLI) let
 * the call through and surfaced a raw 404 with a misleading "verify the name exists" hint.
 */
describe('ensureServerDrivenSupport', () => {
  /** Discovery doc served by the mock: `has` controls whether ddic/desd is advertised. */
  function mockHttp(opts: { loaded: boolean; has?: boolean; getThrows?: boolean }) {
    ...
    return {
      http: {
        ...
        discoveryAcceptFor: (p: string) =>
          opts.loaded && p === '/sap/bc/adt/ddic/desd' ? 'application/vnd.sap.adt.blues.v1+xml' : undefined,
  ...
  it('fetches discovery when cold, then reports the type as unavailable (pre-8.16)', async () => {
  ...
  it('fetches discovery when cold, then reports the type as available (8.16)', async () => {
...
describe('SDO registry write metadata', () => {
  ...
  it('EVTO and UIAD use blues v2; the others use v1 (verified live on 816)', () => {
    expect(SDO_REGISTRY.EVTO.blueContentType).toContain('v2');
    expect(SDO_REGISTRY.UIAD.blueContentType).toContain('v2');
...
describe('buildBlueSourceXml', () => {
  ...
  it('never emits adtcore:masterLanguage — ADT ignores it for these objects (live-verified 816)', () => {
    ...
    for (const code of ['DESD', 'DTSC', 'CSNM', 'EVTB', 'EVTO', 'COTA', 'UIAD']) {
