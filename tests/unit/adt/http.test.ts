...
const mockClientCreated = vi.fn();

class MockClient {
  constructor() {
    mockClientCreated(this);
  }
...
describe('AdtHttpClient', () => {
  ...
  describe('proxy configuration', () => {
    ...
    it('reuses one Connectivity proxy Client across requests in a stateful session', async () => {
      mockClientRequest
        .mockResolvedValueOnce(
          mockClientResponse(
            200,
            '<LOCK_HANDLE>LH1</LOCK_HANDLE>',
            { 'set-cookie': ['SAP_SESSIONID_A4H_100=FRESH; Path=/'] },
          ),
        )
        .mockResolvedValueOnce(mockClientResponse(200, ''));
      ...
      // Avoid an extra CSRF bootstrap request so this test contains
      // exactly LOCK + one stateful follow-up request.
      (client as unknown as { csrfToken: string }).csrfToken = 'T';

      await client.withStatefulSession(async (session) => {
        await session.post('/sap/bc/adt/programs/programs/ZX?_action=LOCK&accessMode=MODIFY');
        await session.put(
          '/sap/bc/adt/programs/programs/ZX/source/main?lockHandle=LH1',
          'REPORT zx.',
          'text/plain',
        );
      });
      ...
      // Both stateful requests must traverse the same
      // Connectivity proxy Client/connection context.
      expect(mockClientCreated).toHaveBeenCalledTimes(1);
      expect(mockClientClose).toHaveBeenCalledTimes(1);
    });
    it('closes the reused Connectivity proxy Client without masking the stateful request error', async () => {
      const originalError = new Error('stateful follow-up failed');
      mockClientClose.mockRejectedValueOnce(new Error('proxy close failed'));
      mockClientRequest
        .mockResolvedValueOnce(
          mockClientResponse(
            200,
      ...
      await expect(
        ...
        message: expect.stringContaining(originalError.message),
      });
    ...
    it('preserves the stateful result when the reused Connectivity proxy Client fails to close', async () => {
      mockClientRequest.mockResolvedValueOnce(mockClientResponse(200, 'worked'));
      mockClientClose.mockRejectedValueOnce(new Error('proxy close failed'));
    ...
    it('uses the reused Connectivity proxy Client for stateful close-session cleanup before closing it', async () => {
      mockClientRequest
        .mockResolvedValueOnce(
          mockClientResponse(
            200,
            '<LOCK_HANDLE>LH1</LOCK_HANDLE>',
            { 'set-cookie': ['sap-contextid=CONTEXT_1; Path=/'] },
          ),
        )
        .mockResolvedValueOnce(mockClientResponse(200, ''))
        .mockResolvedValueOnce(
          mockClientResponse(
            200,
            '',
            { 'set-cookie': ['sap-contextid=0; Path=/'] },
          ),
        );
      ...
      expect(mockClientRequest).toHaveBeenCalledTimes(3);
      expect(mockClientCreated).toHaveBeenCalledTimes(1);
      expect(new URL(clientRequestPath(2)).pathname).toBe('/sap/bc/adt/core/http/sessions');
      expect(clientRequestHeaders(2)).toMatchObject({
        'X-sap-adt-sessiontype': 'stateless',
        'sap-adt-purpose': 'close-session',
        'sap-contextid': 'CONTEXT_1',
      });
      expect(clientRequestHeaders(2).Cookie).toContain('sap-contextid=CONTEXT_1');
      expect(mockClientClose).toHaveBeenCalledTimes(1);

      const cleanupOrder = mockClientRequest.mock.invocationCallOrder[2]!;
      const closeOrder = mockClientClose.mock.invocationCallOrder[0]!;
      expect(closeOrder).toBeGreaterThan(cleanupOrder);
    });

    it('keeps discardResponseBody on a dedicated Client inside a stateful session', async () => {
      mockClientRequest
        .mockResolvedValueOnce(mockClientResponse(200, 'discard me'))
        .mockResolvedValueOnce(mockClientResponse(200, 'normal'));
      ...
      await client.withStatefulSession(async (session) => {
        await session.get('/control', undefined, { discardResponseBody: true });
        await session.get('/normal');
      });

      expect(mockClientRequest).toHaveBeenCalledTimes(2);

      // headers-only/discard ownership belongs to connectivityProxyResponse(),
      // so it must never own the Client reused by ordinary stateful requests.
      expect(mockClientCreated).toHaveBeenCalledTimes(2);
      expect(mockClientDestroy).toHaveBeenCalledTimes(1);

      // The ordinary stateful Client survives until withStatefulSession() exits.
      expect(mockClientClose).toHaveBeenCalledTimes(1);
    });
