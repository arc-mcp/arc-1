...
describe('AdtHttpClient', () => {
  ...
  describe('stateful sessions', () => {
    it('emits the configured stateless session header', async () => {
      mockFetch.mockResolvedValueOnce(mockResponse(200, 'closed'));

      const client = new AdtHttpClient({ ...getDefaultConfig(), sessionType: 'stateless' });
      await client.get('/sap/bc/adt/core/http/sessions');

      expect(fetchHeaders(0)['X-sap-adt-sessiontype']).toBe('stateless');
    });

    it('emits the configured stateless session header while fetching CSRF', async () => {
      mockFetch.mockResolvedValueOnce(mockResponse(200, '', { 'x-csrf-token': 'T' }));
    ...
    it('closes the backend context after the stateful callback', async () => {
      mockFetch.mockResolvedValueOnce(mockResponse(200, 'locked', {}, ['sap-contextid=CONTEXT_1; Path=/; HttpOnly']));
      mockFetch.mockResolvedValueOnce(mockResponse(200, '', {}, ['sap-contextid=0; Path=/; HttpOnly']));
      ...
      expect(mockFetch).toHaveBeenCalledTimes(2);
      expect(fetchOptions(1).method).toBe('GET');
      expect(fetchUrl(1)).toContain('/sap/bc/adt/core/http/sessions');
      expect(fetchHeaders(1)).toMatchObject({
        Accept: '*/*',
        'sap-adt-purpose': 'close-session',
        'sap-contextid': 'CONTEXT_1',
        'X-sap-adt-sessiontype': 'stateless',
      });
      expect(fetchHeaders(1).Cookie).toContain('sap-contextid=CONTEXT_1');
    });

    it('closes the backend context after a callback error without replacing that error', async () => {
      ...
      expect(fetchUrl(1)).toContain('/sap/bc/adt/core/http/sessions');
      expect(fetchHeaders(1)['X-sap-adt-sessiontype']).toBe('stateless');
    });

    it('does not fail a successful callback when backend context cleanup fails', async () => {
      const { logger } = await import('../../../src/server/logger.js');
      const warnSpy = vi.spyOn(logger, 'warn').mockImplementation(() => undefined);
      mockFetch.mockResolvedValueOnce(mockResponse(200, 'opened', {}, ['sap-contextid=CONTEXT_3; Path=/']));
      mockFetch.mockResolvedValueOnce(mockResponse(500, 'close failed'));
      const client = new AdtHttpClient(getDefaultConfig());
      ...
      expect(warnSpy).toHaveBeenCalledWith('Failed to close stateful ADT session.', expect.any(Object));
      warnSpy.mockRestore();
    });

    it('falls back to a stateless discovery transition when the close resource is absent', async () => {
      const { logger } = await import('../../../src/server/logger.js');
      const warnSpy = vi.spyOn(logger, 'warn').mockImplementation(() => undefined);
      ...
      // NW 7.50 resets the context cookie but returns 400 after the stateless transition.
      mockFetch.mockResolvedValueOnce(mockResponse(400, '', {}, ['sap-contextid=0; Path=/']));
      const client = new AdtHttpClient(getDefaultConfig());
      ...
      expect(result).toBe('written');
      expect(fetchOptions(2).method).toBe('HEAD');
      expect(fetchUrl(2)).toContain('/sap/bc/adt/core/discovery');
      expect(fetchHeaders(2)['X-sap-adt-sessiontype']).toBe('stateless');
      expect(warnSpy).not.toHaveBeenCalled();
      warnSpy.mockRestore();
    });

    it('does not send a close request when SAP did not establish a context', async () => {
      mockFetch.mockResolvedValueOnce(mockResponse(200, 'stateless response'));
      const client = new AdtHttpClient(getDefaultConfig());

      await client.withStatefulSession(async (session) => {
        await session.get('/no-context');
      });
