...
| `SAP_BLOCKED_DATA_SOURCES` | Experimental exact-name data-source denylist (default empty/off). When active, data reads use strict SQL parsing + live CDS/replacement lineage and fail closed if lineage cannot be proven. |
...
| Experimental SQL/data source denylist | `src/adt/{sql-source-analyzer,data-source-policy}.ts`, `src/adt/client.ts`, `src/server/{config,types}.ts`, tests — exact names only; empty is zero-overhead; active mode checks live CDS/replacement lineage and refuses unsupported/unresolved sources before data-preview POST |
