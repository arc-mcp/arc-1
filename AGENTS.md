...
| SKTD/KTD multi-node docs read/write | `src/adt/ddic-xml.ts` (`decodeKtdText`/`rewriteKtdText`), `src/handlers/read.ts` + `write/{create,update-delete}.ts` — a KTD holds one `<sktd:element>` per node (root id = object name; every other id = an ADT fragment URI), each with Base64 `<sktd:text>`. Exact full `## <node id>` headings are reserved routing syntax (case-insensitive ABAP spelling); full SAPRead reversibly prefixes colliding body headings and exact `KTD_META_MARKER` body lines with `\`, and a lone root-name H2 without SAPRead context is refused as ambiguous. Unaddressed nodes stay byte-identical; an unchanged documented non-writable section is preserved, while attempted non-documentable, empty-body, and unaddressed 2+-documented-node writes are REFUSED. The write's `getKtd()` deliberately takes no `version` so consecutive drafts accumulate. `SAPRead` heads a lone documented node when writable empty siblings exist, then puts its writable-node index and every version/cache annotation behind the final unescaped `KTD_META_MARKER`; `SAPContext` and `grep` stay on unescaped bare Markdown. Never synthesize an `<sktd:element>`. Details: docs/research/2026-09-02-sktd-multi-node-write.md |
