...
| Enterprise security checks / source dependency evidence | `docs/security-operations.md`, `.github/security-ruleset.json`, `scripts/security/{evidence,summary}.mjs`, `.github/workflows/dependency-evidence.yml` — security-only merge requirements; ruleset starts disabled until release-App/check verification; source inventories never claim final CF staging coverage |
