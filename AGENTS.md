...
| Headless CI ATC/AUnit (`SAPDiagnose atc_ci`/`unittest_ci`) | `src/adt/ci-quality.ts` + `ci-quality-xml.ts`, `diagnose.ts`, `{schemas,tools}.ts`, `policy.ts` — Communication APIs (not IDE worklists); CSRF dummy UUID; product-neutral; CLI exits 1 when `fail` |
