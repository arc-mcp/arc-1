...
| SKTD/KTD multi-node docs read/write | `src/adt/ddic-xml.ts` (`decodeKtdText`/`rewriteKtdDocument`), `src/handlers/read.ts` + `write/{create,update-delete}.ts` — a KTD holds one `<sktd:element>` per node (root id = object name; every other id = an ADT fragment URI), each with Base64 `<sktd:text>` and optionally `<sktd:shortText sktd:text="Base64" sktd:obligation="…"/>`. Bodies (`## <node>` sections) and `shortTexts[].node` address a node by ONE shared resolver: exact full id → unique case-insensitive id → unique node name as the SAPRead index prints it (percent-decoded or wire-encoded), against every node without collapsing distinct IDs; duplicate-ID envelopes remain readable but writes are refused, and ambiguous aliases are refused, a bare last segment (`## Update`) stays prose because every BDEF has `<Entity>.update`, a node-shaped heading (ADT URI, `#type=`, known qualifier, object namespace, or a copied `(empty)` marker) that matches nothing aborts the whole write, and create/update replies report changed nodes plus headings kept as prose (`KtdWriteReport`). Full SAPRead reversibly prefixes body headings that equal a route — id or name — and exact `KTD_META_MARKER` body lines with `\`; a lone root-name H2 without SAPRead context is refused as ambiguous. Unaddressed nodes stay byte-identical; unchanged documented non-writable sections pass through, while attempted non-documentable, empty/ambiguous body, and forbidden/missing short-text writes are REFUSED. The write's `getKtd()` deliberately takes no `version` so consecutive drafts accumulate. `SAPRead` heads a lone documented node when writable empty siblings exist, then puts populated short texts, its index of EVERY writable node (`formatKtdNodeIndex`, each by the spelling that resolves back to it, empty ones listed on an `empty (n):` line), and version/cache annotations behind the final unescaped `KTD_META_MARKER`; SAPWrite `dryRun` validates and reports changed/untouched nodes without a PUT; `SAPContext` and `grep` stay on unescaped bare Markdown. Never synthesize an `<sktd:element>` or `<sktd:shortText>`. Details: docs/research/2026-09-02-sktd-multi-node-write.md, docs/research/2026-09-04-sktd-short-texts.md, and docs/research/2026-09-08-pr766-ktd-routing-review.md |
