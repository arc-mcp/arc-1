...
| Trap | Reality |
|------|---------|
| Putting BTP config in `.env` | On CF, config lives in `mta-overrides.mtaext` `properties:` (or `cf set-env`). `.env*` is stripped from the archive by BOTH `.cfignore` and `mta.yaml` `ignore:`, so a `.env` deploy yields a **healthy app with no SAP target** — see [docs_page/configuration-precedence.md](docs_page/configuration-precedence.md) |
...
| Assuming MTA reuses pre-existing services | `arc1-destination`/`arc1-connectivity` are `managed-service`, whose instance name defaults to the RESOURCE name — so a space with differently-named instances gets ADDITIONAL ones, not reuse. With the subaccount-level destinations this project recommends the new instance still resolves the same targets, so the usual cost is `lite` quota (the deploy FAILS on a subaccount at its limit) and landscape confusion; it breaks outright only if the shared instance carries instance-level destinations. Run `cf services` first. An extension CANNOT change a resource's `type` (`mbt` rejects `type` in `mta.ResourceExt`), only repoint it with `service-name:` — which leaves MTA owning that instance |
