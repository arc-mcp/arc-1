...
| SKTD/KTD multi-node docs read/write | `src/adt/ddic-xml.ts` (`decodeKtdText`/`rewriteKtdText`), `src/handlers/read.ts` + `write/{create,update-delete}.ts` — a KTD holds one `<sktd:element>` per documented node (root id = object name; every other id = the element's ADT fragment URI), each with a Base64 `<sktd:text>`. Exact full `## <node id>` headings are reserved routing syntax (case-insensitive ABAP spelling); unaddressed nodes stay byte-identical, empty bodies and unaddressed bodies over 2+ documented nodes are REFUSED. The write's `getKtd()` deliberately takes no `version` so consecutive draft writes accumulate. `SAPRead` puts its compact empty-node index and every version/cache annotation behind the exact `KTD_META_MARKER`; the writer strips that trailer, while `SAPContext` and `grep` stay on bare Markdown. Never synthesize an `<sktd:element>` — the child schema is unconfirmed. Details: docs/research/2026-09-02-sktd-multi-node-write.md |
