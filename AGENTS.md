...
| Optional SAPGraph | `src/repository-graph/`, server/dispatch seams; schema derives from `contract.ts`. Read ADR-0008 first: explicitly shared single-system metadata only, never SAP PP credentials/cache fallback. |
| Optional graph backend | `services/repository-graph/` (own install/build/tests), `docs_page/repository-graph-backend.md` — PostgreSQL/HANA metadata only; default ARC distribution must not depend on it. |
