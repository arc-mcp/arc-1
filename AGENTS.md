...
| Release npm SBOM | `.github/workflows/release.yml` (`publish-npm-sbom`), `tests/unit/server/release-sbom-workflow.test.ts`, `docs_page/security-guide.md` — production lockfile graph only; check out the release tag, keep npm pinned to `publish-npm`, validate version/shape, and never use destructive asset replacement |
