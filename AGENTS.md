...
| Transport review diff (`SAPTransport action="diff"`) | `src/adt/transport-diff.ts` (pair selection + LIMU→R3TR rollup), `src/handlers/transport.ts`, `src/adt/xml-parser.ts` (`revisionTransportId`) — the versions feed carries the CTS id in `adtcore:name` on a `…/relations/transport/request` link, NOT in the link title (that is the description); `<atom:id>` is the version number, `00000` = active. Match request ∪ tasks (versions reference either — live-verified). Entries can lack `<atom:updated>` → sort undated as oldest. Evidence + SAP's own algorithm: docs/plans/2026-08-03-transport-diff.md |
