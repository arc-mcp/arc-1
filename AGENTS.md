...
| SKTD/KTD multi-node docs read/write | `src/adt/ddic-xml.ts` (`decodeKtdText`/`rewriteKtdText`), `src/handlers/read.ts` + `write/{create,update-delete}.ts` — a KTD holds one `<sktd:element>` per documented node (root id = object name; every other id = the element's ADT fragment URI), each with a Base64 `<sktd:text>`. `rewriteKtdText` is the exact inverse of the reader: `## <node id>` addresses one node (ids match case-insensitively — the root id is upper-cased on the wire), unaddressed nodes stay byte-identical, empty bodies and unaddressed bodies over 2+ documented nodes are REFUSED (the latter used to overwrite the root and drop the rest). The write's `getKtd()` deliberately takes no `version` so consecutive draft writes accumulate. `SAPRead` appends a compact index of the nodes SAP pre-created without text (root name + `<base>#type=T;name=N` groups — `formatKtdUndocumentedIndex`) so their ids are discoverable; `SAPContext` and `grep` stay on the bare Markdown. Never synthesize an `<sktd:element>` — the child schema is unconfirmed. Short texts: SAPWrite `shortTexts=[{node,text}]` (60 chars, refused on root/BAE `forbidden`; resolver exact→case→unique node name, qualified or short, decoded or wire-encoded — `resolveKtdNodeIn`; `## ` headings share it but accept only the qualified spellings, since every BDEF has `<Entity>.update` and `## Update` must stay prose); SAPRead lists them in a trailer behind `KTD_META_MARKER` that `stripKtdMetaTrailer` removes before either write path parses the body; entry point for both is `rewriteKtdDocument`. SAP derives `adtcore:description` from the short text (live-verified) — never write it. Details: docs/research/2026-09-02-sktd-multi-node-write.md |
