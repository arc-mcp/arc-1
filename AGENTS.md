...
| Letting MTA adopt pre-existing services | `arc1-destination`/`arc1-connectivity` are `managed-service`, whose instance name defaults to the RESOURCE name — in a landscape whose shared Destination instance is named differently this silently creates a SECOND, empty instance and binds ARC-1 to it. Run `cf services` first. An extension CANNOT change a resource's `type` (`mbt` rejects `type` in `mta.ResourceExt`) — it can only repoint a managed resource with `service-name:`, which leaves MTA owning that instance. Binding a separately-owned shared instance needs `org.cloudfoundry.existing-service` in `mta.yaml` itself |
